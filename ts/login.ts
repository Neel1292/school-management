...
function getTeacherLogin(email:string, password:string):boolean {
    
    const teacher = localStorage.getItem('teacherData');
    let teacherCred = teacher ? JSON.parse(teacher) : [];
    let name ;
    let matchResult = teacherCred.filter((teacher:any) => {
            if(teacher.email === email ) name = teacher.name;
...
function getStudentLogin(email:string, password:string):boolean {
    const students = localStorage.getItem('studentData');
    let studentCred = students ? JSON.parse(students) : [];
    let name ;
    let matchResult = studentCred.filter((student:any) => {
            if(student.email === email ) name = student.name;
