...
var displayData = document.querySelector(".question-table-body");
const questionsViewForm = document.querySelector(".question-data-view");
const questionsTableView = document.querySelector(".question-table-container");
const searchQuestions = document.querySelector("#question-search");
var refreshButton = document.querySelector(".fa-arrows-rotate");
refreshButton.addEventListener("click", () => {
    window.location.reload();
});
getQuestionView();
searchQuestions.addEventListener("keypress", (event) => {
    // debugger
    ...
    if (searchValue) {
        questionData = questionData.filter((questions) => {
            return (questions.author.toLowerCase().includes(searchValue.toLowerCase()) ||
...
function deleteQuestion(index) {
    let teacher = questionDataJSON ? JSON.parse(questionDataJSON) : [];
